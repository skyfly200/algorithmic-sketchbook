...
// Per-node-type renderers. Each draws one node into its offscreen ctx (octx of
// its runtime state s), and is looked up from NODE_RENDERERS by evalNode. They
// stay in the component so they keep live access to W/H and the compositor
// helpers (pval, inputCanvas, cover, …); geo/vcam/geodata delegate to their own
// evaluators.
...
function renderSprite(node, octx) {
...
function renderPortal(node, octx, s) {
  const input = inputCanvas(node, 0)
...
function renderMask(node, octx, s) {
...
function renderPolygon(node, octx) {
...
function renderBlend(node, octx) {
...
function renderOutput(node, octx) {
  const input = inputCanvas(node, 0)
  if (input) octx.drawImage(input, 0, 0, W, H)
}
function renderInput(node, octx) {
...
function renderTracker(node, octx, s) {
...
// Dispatch table: node type → renderer. evalNode clears the canvas then calls
// the matching one (geo/vcam/geodata keep their dedicated evaluators).
...
function evalNode(node) {
  ...
  octx.fillStyle = '#000'
  octx.fillRect(0, 0, W, H)
  NODE_RENDERERS[node.type]?.(node, octx, s)
}
