...
const params = rt.params({
  mode: { value: 'Bonfire', type: 'select', options: ['Bonfire', 'Campfire', 'Torch', 'Candle', 'Blue Gas'], label: 'Mode' },
...
function modeProfile(x, t) {
  ...
  if (m === 'Torch') {
    // a licking column with turbulent sway
    const cx = 0.5 + Math.sin(t * 2.2) * 0.05 + Math.sin(t * 5) * 0.02
    const w = 0.05
    return Math.exp(-((f - cx) ** 2) / (w * w)) * 1.05
  }
  if (m === 'Blue Gas') {
    // a burner: a row of tight blue jets over the ports
    let v = 0
    for (const c of [0.34, 0.5, 0.66]) v += Math.exp(-((f - c) ** 2) / (0.02 * 0.02))
    return Math.min(1, v) * 1.05
  }
  if (m === 'Campfire') {
    // a bed of coals with a few tighter tongues over the "logs"
    const bed = f > 0.24 && f < 0.76 ? 0.7 : 0.15
    const tongues = 0.5 * (Math.max(0, Math.sin(f * 26 + t * 3)) ** 4)
    return Math.min(1, bed + tongues)
  }
  // Bonfire: a clustered column rather than a flat wall — reads as flames, not
  // a sheet of fire.
  return 0.55 + 0.45 * Math.pow(Math.max(0, Math.sin(f * 14 + t * 2)), 2)
}
// Per-mode biases (intensity/ember/cool) and a base hue-rotate so each mode has
// its own flame colour — warm orange for wood fires, hot blue for the gas jet.
const modeBias = {
  Bonfire: { intensity: 1, ember: 1, cool: 1, hue: 0 },
  Campfire: { intensity: 0.9, ember: 1.5, cool: 1.05, hue: 4 },
  Torch: { intensity: 1.05, ember: 0.7, cool: 0.95, hue: 8 },
  Candle: { intensity: 0.7, ember: 0.25, cool: 1.15, hue: 14 },
  'Blue Gas': { intensity: 1.1, ember: 0.08, cool: 0.82, hue: 205 },
}
...
function frame(now) {
  ...
  for (let y = 0; y < GH - 1; y++) {
    for (let x = 0; x < GW; x++) {
      ...
      // Weight the cell directly below much more than its neighbours so heat
      // rises in tall columns instead of spreading into a flat sheet.
      let v = (below * 4 + l + r) / 6 - cool - rt.rng() * 0.015
  ...
  // per-mode base hue-rotate + the user Hue shift (blue for the gas jet)
  ctx.filter = `hue-rotate(${params.hue - 20 + (bias.hue ?? 0)}deg)` +
    (params.mode === 'Blue Gas' ? ' saturate(1.3)' : '')
  ...
  if (emberRate > 0.01) {
    ...
    for (let i = embers.length - 1; i >= 0; i--) {
      ...
      const emberHue = params.mode === 'Blue Gas' ? 200 : 30 + params.hue + rt.rng() * 20
      ctx.fillStyle = `hsla(${emberHue}, 100%, 65%, ${e.life})`
