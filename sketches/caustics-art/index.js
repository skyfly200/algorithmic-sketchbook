...
// Imported into the Bright Waves sketchbook: model URLs are resolved through the
// bundler (?url) so the .obj assets are emitted and cached with the page; THREE
// is still the global from the CDN <script> tags in index.html.
import sharkUrl from './assets/WhiteShark.obj?url'
import rockUrl from './assets/rock.obj?url'
import plantUrl from './assets/plant.obj?url'
...
const sharkLoaded = new Promise((resolve) => {
  objLoader.load(sharkUrl, (sharkGeometry) => {
...
const rockLoaded = new Promise((resolve) => {
  objLoader.load(rockUrl, (rockGeometry) => {
...
const plantLoaded = new Promise((resolve) => {
  objLoader.load(plantUrl, (plantGeometry) => {
